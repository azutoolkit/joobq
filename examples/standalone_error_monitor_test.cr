...
# Standalone test for error monitoring without Redis dependency
puts "=== Standalone JoobQ Error Monitoring Test ==="
puts
...
puts
puts "10. Testing MonitoredErrorHandler..."

# Create a mock job and queue for testing
class MockJob
  include JoobQ::Job
...
class MockQueue
  ...
  getter name : String = "mock-queue"
  getter total_workers : Int32 = 5
  getter running_workers : Int32 = 3
  getter job_type : String = "MockJob"
...
# Test MonitoredErrorHandler
job = MockJob.new
queue = MockQueue.new
exception = RuntimeError.new("Test error")

error_context = JoobQ::MonitoredErrorHandler.handle_job_error(
  job,
  queue,
  exception,
...
puts "  ✓ MonitoredErrorHandler processed error"
puts "  Job ID: #{error_context.job_id}"
puts "  Queue: #{error_context.queue_name}"
puts "  Error Type: #{error_context.error_type}"
puts "  Worker ID: #{error_context.worker_id}"

# Verify error was recorded in monitor
stats = JoobQ.error_monitor.get_error_stats
puts "  Error recorded in monitor: #{stats.any?}"

puts
puts "=== Standalone Error Monitoring Test Completed Successfully! ==="
puts
...
puts "  ✓ MonitoredErrorHandler integration"
